...
4. **The daemon runs from its own git checkout** (`~/.steward/src`), per BRIEF promise 6.
   There is no compiled release artifact in v1; "install" = clone + `bun install` + build UI.
...
├── src/                    # git checkout of steward source (the running code)
│   ├── package.json
│   ├── daemon/             #   Bun/Hono daemon entry: daemon/main.ts
│   ├── ui/                 #   React+Vite+Tailwind source
│   └── ui/dist/            #   built static assets, served by daemon (gitignored)
├── bin/
│   └── steward             # CLI shim (bash script, §7)
├── env                     # KEY=VALUE overrides sourced by shim & daemon (§2.1)
├── daemon.pid              # written by daemon on boot (pid + boot-id line)
├── daemon.sock             # (later) unix socket for CLI→daemon fast path; v1 uses HTTP
├── logs/
│   ├── daemon.log          # stdout, rotated by daemon (10MB x 5)
...
### Step 3 — Clone or update source at `~/.steward/src`

```
if [ ! -d ~/.steward/src/.git ]; then
  rm -rf ~/.steward/src                       # kill any half-clone debris
  git clone --branch "$CHANNEL" "$REPO" ~/.steward/src
else
  git -C ~/.steward/src remote set-url origin "$REPO"
  git -C ~/.steward/src fetch origin "$CHANNEL"
  if repo_is_dirty; then
    log "WARN: ~/.steward/src has local changes; skipping source update"
    # dev machines hack on their own steward; never clobber. Build proceeds from HEAD.
  else
    git -C ~/.steward/src checkout "$CHANNEL"
    git -C ~/.steward/src reset --hard "origin/$CHANNEL"
  fi
fi
...
$BUN run build          # package.json script: vite build ui -> ui/dist
$BUN run selfcheck      # daemon/main.ts --selfcheck: loads modules, opens throwaway
...
running service — an existing healthy daemon keeps running on its old code (§5).
...
```
if curl -fsS --max-time 2 "http://127.0.0.1:$PORT/api/health" | grep -q '"steward"'; then
...
`/api/health` returns `{"app":"steward","version":...,"commit":...}` — the `"steward"`
...
Poll `http://127.0.0.1:$PORT/api/health` every 250ms for up to 30s. On success, verify the
...
<plist version="1.0"><dict>
  ...
  <array>
    <string>/Users/eric/.bun/bin/bun</string>
    <string>run</string>
    <string>/Users/eric/.steward/src/daemon/main.ts</string>
  </array>
  <key>WorkingDirectory</key><string>/Users/eric/.steward/src</string>
...
[Service]
Type=simple
ExecStart=/home/eric/.bun/bin/bun run /home/eric/.steward/src/daemon/main.ts
WorkingDirectory=/home/eric/.steward/src
...
|---|---|
| `~/.steward/src` | fetch+reset if clean; untouched (warn) if dirty |
| `node_modules` | `bun install` is naturally idempotent |
| `ui/dist` | rebuilt every run (cheap, ~seconds with vite) |
| `bin/steward` | overwritten every run (shim is versionless glue) |
...
| `steward.db`, `keys/`, `blobs/` | **never touched by the installer** |
...
4. Data:
   default            -> keep ~/.steward, print "your data (db, keys, blobs) kept at
...
`127.0.0.1:$STEWARD_PORT` (auth: a `token` file `~/.steward/cli.token`, chmod 600, written
by the daemon at boot and sent as `Authorization: Bearer` — filesystem permission *is* the
auth boundary for local CLI). Heavy subcommands delegate to
`$STEWARD_BUN run ~/.steward/src/cli/main.ts <args>` so real logic lives in TypeScript.

```
steward status      GET /api/health + service-manager state. Shows: running/stopped, pid,
...
steward restart     POST /api/admin/restart (daemon finishes in-flight work, exits 64,
                    service manager relaunches). Falls back to kickstart -k if API down.
steward update      bash ~/.steward/src/install.sh --channel "$STEWARD_CHANNEL"
                    (after a 'git fetch && reset' of src if clean — so the newest
                    installer runs; see §5). '--check' flag: just print behind-by count.
...
steward pair        delegates to cli/main.ts pair — prints this node's short pairing code
                    ...
                    peer. Thin veneer over POST /api/pairing/* (identity design doc owns
                    the protocol).
steward uninstall   §6
...
`EADDRINUSE` it GETs `/api/health`; if the responder is a steward with a *different* pid
file boot-id, it exits with code 65 ("another steward owns this port") — and both launchd
...
- `steward update` and the daemon's own "Update & Restart" UI button both execute
  `bash ~/.steward/src/install.sh` (freshly fetched), i.e. update == reinstall.
- Daemon-initiated update spawns the installer **detached** (`setsid`/`nohup`, output to
  ...
  polls `/api/health` and shows the version change.
- Rollback: `git -C ~/.steward/src reset --hard <good-commit> && steward restart` — plus
  a `steward update --to <commit>` convenience flag. The installer records the previous
  commit in `install.meta.json.previousCommit` for `steward update --rollback`.
